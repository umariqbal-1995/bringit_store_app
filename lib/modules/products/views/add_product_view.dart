...
class AddProductView extends StatefulWidget {
  ...
  const AddProductView({super.key, this.isEditing = false});
...
class _AddProductViewState extends State<AddProductView> {
  ...
  void initState() {
    ...
    final product = widget.isEditing ? controller.selectedProduct.value : null;
    ...
    isAvailable = (product?.isAvailable ?? true).obs;
  ...
  Future<void> _save() async {
    ...
    if (_imageFile != null) {
      ...
      try {
        imageUrl = await FirebaseStorageService.uploadFile(_imageFile!, 'products');
      } catch (_) {
    ...
    final data = <String, dynamic>{
      ...
      'pricePkr': double.tryParse(priceCtrl.text) ?? 0,
      'isActive': isAvailable.value,
    };
    if (descCtrl.text.trim().isNotEmpty) data['description'] = descCtrl.text.trim();
    if (categoryCtrl.text.trim().isNotEmpty) data['category'] = categoryCtrl.text.trim();
    if (imageUrl != null) data['imageUrl'] = imageUrl;

    final product = widget.isEditing ? controller.selectedProduct.value : null;
    if (widget.isEditing && product != null) {
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(controller: nameCtrl, decoration: _inputDecoration('e.g. Chicken Burger')),
                  const SizedBox(height: 16),
                  _fieldLabel('Price (PKR) *'),
                  const SizedBox(height: 6),
                  TextField(
                  ...
                  const SizedBox(height: 6),
                  TextField(controller: descCtrl, maxLines: 3, decoration: _inputDecoration('Optional description...')),
                  const SizedBox(height: 16),
                  _fieldLabel('Category'),
                  const SizedBox(height: 6),
                  TextField(controller: categoryCtrl, decoration: _inputDecoration('e.g. Burgers, Drinks...')),
                  const SizedBox(height: 16),
                  ...
                  Row(
                    ...
                    children: [
                      const Text('Available', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                      Obx(() => Switch(
